...
binded(cast, {send, {list, To, Payload}}, #data{socket = Socket}) ->
  ...
  JSONPayload = lists:foldl(
    fun({Key,Value},Acc) -> maps:put(Key, Value,Acc) end,#{<<"message_id">> => base64:encode(crypto:strong_rand_bytes(6)), <<"to">> => To, <<"delivery_receipt_requested">> => 'true'},Payload),
  send_push(jsone:encode(JSONPayload), Socket),
  {keep_state_and_data, []};

binded(cast, {send, {list, To, Payload, Params}}, #data{socket = Socket}) ->
  error_logger:info_msg("Received request to send push payload (list) :~p",[Payload]),
  JSONPayload = lists:foldl(
    fun({Key,Value},Acc) -> maps:put(Key, Value,Acc) end,#{<<"message_id">> => base64:encode(crypto:strong_rand_bytes(6)), <<"to">> => To, <<"delivery_receipt_requested">> => 'true'}, Payload ++ Params),
  send_push(jsone:encode(JSONPayload), Socket),
